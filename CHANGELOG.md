...
- **限流错误不再被当成永久失败**：飞书对发送过快的机器人返回 HTTP 200 +
  business code `11232`（`frequency limited psm[...]appID[...]`），而不是 429。
  `is_retryable()` 只认 `Http` 与 5xx，于是限流被归类为永久错误，
  重试一次都不做，告警直接丢弃。
  - `is_retryable()` 补上 429 与 business code `11232`；
    非限流的业务错误（如签名错误）仍然不重试。
  - 新增 `is_rate_limited()`，限流单独走秒级退避（1s 起步、上限 10s）：
    飞书按分钟限流，而调用方默认的 100/200/400ms 会把每次重试
    都打在同一个限流窗口里，三次重试全部白费。该退避只拉长
    调用方配置的退避，绝不缩短。
  - `backoff()` 因此需要错误类型，签名增加一个参数（私有方法）。

`is_retryable` / `is_rate_limited` 均为私有函数，公共 API 未变。
